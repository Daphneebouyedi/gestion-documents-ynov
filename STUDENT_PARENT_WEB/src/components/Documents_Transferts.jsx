...
const DocumentsTransferes = () => {
  ...
  const handleDelete = (doc) => {
    if (window.confirm(`Supprimer le document transféré ?`)) {
      setDocuments(docs => docs.filter(d => d._id !== doc._id));
    }
  };

  return (
    <DashboardLayout pageTitle="Documents transférés" pageDescription="Consultez et gérez les documents transférés">
        {/* Filtres */}
        <div
          style={{
        ...
        <table style={{ width: '100%', background: '#fff', borderRadius: 16, boxShadow: '0 2px 8px rgba(0,0,0,0.06)', overflow: 'hidden' }}>
          ...
          <tbody>
            {filteredDocuments.length > 0 ? (
              ...
              <tr>
                <td colSpan={8} style={{ textAlign: 'center', color: '#888', padding: 24 }}>Aucun document transféré trouvé.</td>
              </tr>
            )}
          </tbody>
        </table>
